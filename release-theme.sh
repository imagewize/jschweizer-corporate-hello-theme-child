...
# WordPress Theme Release Script with AI Integration (Claude/Codex/Vibe)
# Automates version bumping and changelog updates for theme releases
# Designed for standalone child theme repositories (theme files at repo root)
#
# Usage:
#   ./release-theme.sh <version> [--commit] [--ai=claude|codex|vibe]
#   ./release-theme.sh 2.1.8
#   ./release-theme.sh 2.2.0 --commit --ai=codex
#
# What it does:
...
if [ -z "$1" ]; then
    ...
    echo "Usage: $0 <version> [--commit] [--ai=claude|codex|vibe]"
    echo "Example: $0 2.1.8"
    echo "Example: $0 2.2.0 --commit --ai=codex"
    exit 1
fi

NEW_VERSION="$1"

# Theme directory is the repo root (standalone child theme repo)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
THEME_DIR="$SCRIPT_DIR"

# Validate theme files exist
if [ ! -f "$THEME_DIR/style.css" ]; then
    echo -e "${RED}Error: style.css not found in $THEME_DIR${NC}"
    echo "Run this script from the theme repo root."
    exit 1
fi
...
# Read theme display name from style.css
THEME_DISPLAY_NAME=$(grep "^Theme Name:" "$THEME_DIR/style.css" | sed 's/Theme Name: *//')
echo -e "${BLUE}=== $THEME_DISPLAY_NAME Release Tool with ${AI_TOOL_DISPLAY} AI ===${NC}"
echo ""

# Get current version from style.css
CURRENT_VERSION=$(grep "^Version:" "$THEME_DIR/style.css" | sed 's/Version: *//')
echo -e "Theme location:  ${BLUE}$THEME_DIR${NC}"
...
GIT_DIFF=$(git -C "$THEME_DIR" diff main..HEAD | head -c 50000)  # Limit to ~50KB
...
# Update functions.php version constant
if [ -f "$THEME_DIR/functions.php" ]; then
    sed -i.bak "s/define( 'HELLO_ELEMENTOR_CHILD_VERSION', '[^']*' );/define( 'HELLO_ELEMENTOR_CHILD_VERSION', '$NEW_VERSION' );/" "$THEME_DIR/functions.php"
...
# Show git diff
git -C "$THEME_DIR" diff style.css readme.txt CHANGELOG.md functions.php
...
echo -e "   ${GREEN}git add style.css readme.txt CHANGELOG.md functions.php${NC}"
...
if [ "$AUTO_COMMIT" = true ]; then
    ...
    git -C "$THEME_DIR" add style.css readme.txt CHANGELOG.md functions.php
    git -C "$THEME_DIR" commit -m "$THEME_DISPLAY_NAME Version $NEW_VERSION"
